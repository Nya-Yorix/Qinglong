...
refresh_token = os.environ.get("Alipan_Refresh_Token", "")
                ...
                if len(sign_in_logs_list) > 0:
                    for i, sign_in_logs_dict in enumerate(sign_in_logs_list, 1):
                        ...
                        if status == "":
                            ...
                            # 获取签到奖励内容
                            #logger.info(f"day={day}, raw reward: {reward}")####
                            if reward:
                                name = reward.get('name', '')
                                # 优先使用 notice，若为空则使用 description
                                notice = reward.get('notice', '')
                                description = reward.get('description', '')
                                # 如果 notice 不为空，则显示 notice 内容；否则显示 description
                                reward_detail = notice if notice else description
                            else:
                                name = '无奖励'
                                reward_detail = ''
                            today_info = '✅' if day == sign_in_count else '☑'
                            log_info = f"{today_info}打卡第{day}天，获得奖励：{name}->{reward_detail}"
